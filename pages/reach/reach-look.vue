	...
	export default {
		...
		methods: {
			...
			chview: function(id) {
				...
				uni.navigateTo({
					url: '../reach/reach-num?id='+id
				})
			},
			onLoad: function (options) {
			console.log('6666664432212356789',options) 
		 	if(options.numid==undefined||options.dateid==undefined){
				this.isShow=false
			} else{
				// this.isShow=true
				for(let i=0;i<this.content.length;i++){
					let obj = this.content[i]
					if(options.enchangeid == obj.id){
						obj.isShow = true
					}
				}
