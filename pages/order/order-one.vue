<template>
	<view class="container">
		...
		<view class="cont">供应商代号：{{options.careCode}}</view>
		...
		<checkbox-group name="check" @change="changeCheck" class="check">
			...
			<view class="checkview" v-for="(item, index) in content" :key="item.value">
				...
				<view class="one" @click="chview(index)"> 
					...
					<view class="oone">计划到料日期：{{item.dueDate}}</view>
<!-- </view> -->
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			chview: function(index) {
				...
				uni.navigateTo({
					url: '../order/order-two?index=' + index + '&lmessage=' + this.content[index].itemCode + "&mmessage=" + this.content[index].dscription + '&wmessage=' + this.content[index].docEntry + "&jmessage=" + this.content[index].lineNum +'&qmessage=' + this.content[index].shipDate + "&ymessage=" + this.content[index].unpaidQuantity+'&arrivalDateAfter='+this.arrivalDateAfter+'&arrivalDateBefore='+this.arrivalDateBefore+"&careCode="+this.careCode+'&itemCode='+this.itemCode+"&orderType="+this.orderType+"&salesmanName="+this.salesmanName
				})
